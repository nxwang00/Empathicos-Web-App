...
@section('title', '| All Character Templates')
...
<div class="content container-fluid">
    ...
    <div class="page-header">
        <div class="row align-items-center">
            <div class="col">
                <h3 class="page-title"> All Character Templates </h3>
                <ul class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ route('admin.app.dashboard')}}">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('admin.badge-templates.index')}}">Character Templates</a></li>
    ...
    <div class="row filter-row">
        ...
        <div class="col-sm-6 col-md-3">
            <div class="form-group form-focus select-focus">
                <select class="select floating" id="searchByBadge">
                    <option value="all"> -- Select Character -- </option>
                    @foreach ($badges as $index => $badge)
                        <option value="{{$badge->id}}">{{$badge->name}}</option>
                    @endforeach
                </select>
                <label class="focus-label">Character</label>
            </div>
        </div>
    </div>
    <!-- /Search Filter -->
...
<script type="text/javascript">
    $(document).ready(function(){
        $('#admin_cards_table').DataTable({
            ...
            order: [[ 0, 'desc' ]],
            ajax:{
                url: "{{ route('admin.badge-templates.index') }}",
                serverMethod: 'post',
                data: function(data){
                    data.badge = $('#searchByBadge').val();
            ...
            columns:[
            {
                ...
                render: function(data, type, full, meta){
                    return `<h2 class="table-avatar-r">
                        <a href="/admin/badge-templates/`+full.id+`" class="avatar-r">
                            <img src="{{ asset('/storage/badge-templates') }}/`+ data + `" class="img-thumbnail" alt="No image Found" width="100" >
                        </a>
                    </h2>`;
                },
            },
            {
                ...
                render: function(data, type, full, meta){
                    return `<h2 class="table-avatar">
                        <a href="/admin/badge-templates/`+full.id+`"  >`
                            +full.badge+`
