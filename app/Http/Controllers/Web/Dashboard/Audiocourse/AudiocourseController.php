...
namespace App\Http\Controllers\Web\Dashboard\Audiocourse;
...
class AudiocourseController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                    ...
                    ->addColumn('action', function($row){
                            ...
                            $btn = editButton('admin.audio-course', $row);
                            $btn .= trashButton('admin.audio-course', $row);
                            return $btn;
                    })
                    ->addColumn('description', function($row){
                     ...
                     ->addColumn('cat_id', function($row){
                        $mainCat = AudioCategory::where('id',$row->cat_id)->first();
                        return $mainCat->title;
                    })
                    ->rawColumns(['action','description'])
                    ->make(true);
        }
        return view('dashboard.pages.audio-course.index', []);
    }
    ...
    {
        $data = AudioCourse::where('id', $id)->first();
        $categories = AudioCategory::All();
        return view('dashboard.pages.audio-course.edit', compact('data','categories'));
    }
    ...
    public function updatedata(Request $request)
    {
        ...
        $mainCat = AudioCourse::findOrFail($request->id);
        ...
        $saveData = $mainCat->save();
        if($saveData){
            flashWebResponse('updated', 'Audio course');
            return redirect()->route('admin.audio-course.index');  
        }
